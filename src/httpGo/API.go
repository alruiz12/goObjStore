...
func PutObjAPI(w http.ResponseWriter, r *http.Request){
	...
	go func() {
		...
		file, err := os.Create(os.Getenv("GOPATH") + "/src/github.com/alruiz12/goObjStore/src/" + addedResults )
		if err != nil {
			fmt.Println(err)
		}
		...
		go PutObjProxy(os.Getenv("GOPATH") + "/src/github.com/alruiz12/goObjStore/src/" + addedResults, conf.TrackerAddr, conf.NumNodes, putOK ,results[1], results[2], results[3], addedResults)
		success := <-putOK
		if success == true {
		...
		os.Remove(os.Getenv("GOPATH") + "/src/github.com/alruiz12/goObjStore/src/" + addedResults )
		if err != nil {
			fmt.Println(err)
		}
	}()
